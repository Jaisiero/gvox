...
#pragma pack(pop)
    ...
    struct Parser {
        ...
        explicit Parser(MagicavoxelParserConfig const &a_config) : config{a_config} {}
    ...
    auto create(void **out_self, GvoxParserCreateCbArgs const *args) -> GvoxResult {
        ...
        auto &self = *(new (std::nothrow) Parser(config));
        *out_self = &self;
    ...
    void iterator_advance(void *self_ptr, void **iterator_ptr, GvoxIteratorAdvanceInfo const *info, GvoxIteratorValue *out) {
        ...
        while (true) {
            ...
            if (std::holds_alternative<ModelInstance>(node)) {
                ...
                if (iter.voxel_index >= model.voxel_count || mode == GVOX_ITERATOR_ADVANCE_MODE_SKIP_BRANCH) {
                    // Exit node
                    iter.offset = model_instance.offset;
                    iter.extent = model_instance.extent;
